...
<body class="w3-animate-opacity">
	...
	<div id="id03" class="w3-half">
			...
			<div class="w3-section">
				<form class="w3-container" action="includes/updatedigitaliseringorder.inc.php" method="post">
						
					<?php 
						$sql ="SELECT * FROM digitalisering WHERE orderid=$orderid";
					...
					<label>Giv en kort beskrivelse af arbejdsopgaven</label>
					...
					<input disabled class="w3-input w3-border w3-hover-border-black w3-section" type="text" name="fakturaemail" value="<?php echo $row['fakturaemail']; ?>">
					...
					<label>Hjemmeside(valgfrit)</label>
					<input class="w3-input w3-border w3-hover-border-black w3-section" type="text" name="hjemmeside" value="<?php echo $row['website']; ?>">
				...
				<form action="includes/uploaddigitalisering.inc.php" method="post" enctype="multipart/form-data">
					<?php echo' <input type="hidden" name="orderid" value='.$orderid.'> ';?>
				...
				<th>Funktion</th>
				<?php 
				$sql = "SELECT * FROM digitaliseringuserfiles WHERE orderid=$orderid;";
				$result = mysqli_query($conn, $sql);
	...
	<div id="delete-modal" class="w3-modal">
		<div class="w3-modal-content w3-card-4 w3-animate-zoom w3-round" style="max-width:600px">
			<div class="w3-center"><br>
				...
				<form action="includes/deletedigitaliseringorder.inc.php" method="post">
				<?php echo' <input type="hidden" name="orderid" value='.$orderid.'> ';?>	
...
<?php 
	$uid = $_SESSION['uid'];
	$sql = "SELECT * FROM digitaliseringcomments WHERE orderid=$orderid;";
	$result = mysqli_query($conn, $sql);
	$resultCheck = mysqli_num_rows($result);
	if ($resultCheck > 0){
...
<div class="w3-padding-16"></div>
	 <form action="includes/createdigitaliseringcomments.inc.php" class="container w3-bottom w3-margin-left" style="position: fixed" method="post">
		 <div class="w3-section">
