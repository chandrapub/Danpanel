...
if(!strpos($_SESSION['email'], '@danpanel.dk') !==false){
	header("Location: ../Danpanel/minprofil.php");
	exit();
}
?>
<head>
		...
		<div class="w3-bar w3-white w3-wide w3-bottombar w3-padding w3-card"> <a href="index.php"><img src="Assets/logo-black.png" width="180" height="40" alt=""></a>
		  ...
		  <a onClick="document.getElementById(\'login\').style.display=\'block\'" 
				...
				?>
						...
						<?php 
						$sql ="SELECT * FROM offentligorders WHERE orderid=$orderid";
	...
	<div id="id03" class="w3-half">
		<div class=" w3-card-4 ">
			...
			<div class="w3-section">
				...
				<form class="w3-container" action="includes/updateorder.inc.php" method="post">
<input disabled class="w3-input w3-border w3-hover-border-black w3-section" type="text" value="<?php echo $row['typeoforder']; ?>"><div class="w3-half">
		  <label>Start Dato</label>
						<input disabled class="w3-input w3-border w3-hover-border-black w3-section" type="datetime" name="startdate" value="<?php echo $row['startdate'];?>" style="border: 0px">
					</div>
					<div class="w3-half">
						<label>Opstartsmøde</label>
						<input disabled class="w3-input w3-border w3-hover-border-black w3-section" type="datetime" name="opstartsdate" value="<?php echo $row['opstartsdate'];?>" style="border: 0px">
					</div>
					<label>Sagsnummer</label>
					<input class="w3-input w3-border w3-hover-border-black w3-section" type="text" name="sagsnr" value="<?php echo $row['sagsnr']; ?>">
					
					<label>Ønsket varighed</label>
					<select class="w3-section w3-input w3-border w3-hover-border-blue" name="duration">
						<option style="display: none" selected value="<?php echo $row['duration']?>" class="w3-bar-item w3-button"><?php echo $row['duration']?> måned(er)</option>
					...
					<input class="w3-input w3-border w3-hover-border-black w3-section" type="text" name="hoursprweek" value="<?php echo $row['hoursprweek']; ?>">
					...
					<select class="w3-section w3-input w3-border w3-hover-border-blue" name="adminuid">
						...
						$sql1 ="SELECT * FROM adminuser WHERE admintype=\"offentlig\" AND adminuid=$adminuid;";
						...
						<option style="display: none" selected value="<?php echo $adminuid;?>" class="w3-bar-item w3-button"><?php echo $row1['adminname'] ;?></option>';
						 <?php
						if(strpos($_SESSION['email'], "ma@danpanel")){
						$sql1 ='SELECT * FROM adminuser WHERE admintype="offentlig";';
						$result1 = mysqli_query($conn, $sql1);
						$resultCheck1 = mysqli_num_rows($result1);
					...
					<textarea value="<?php echo $row['description']; ?>" rows="4" cols="50" class="w3-input w3-border w3-hover-border-black w3-section" name="description"><?php echo $row['description']; ?></textarea>
					<?php 
					if(strpos($_SESSION['email'],"@danpanel.dk")!==false){
				...
				<form action="includes/uploadoffentlig.inc.php" method="post" enctype="multipart/form-data">
					<?php echo' <input type="hidden" name="orderid" value='.$orderid.'> ';?>
				...
				<th>Funktion</th>
				<?php 
				$sql = "SELECT * FROM offentliguserfiles WHERE orderid=$orderid;";
				$result = mysqli_query($conn, $sql);
				...
				<div class="w3-container">
					<button class="w3-input w3-border w3-hover-border-black w3-right w3-button w3-red w3-round" onClick="document.getElementById('delete-modal').style.display='block'">Slet ordre</button>
				</div>
				...
				<div class="w3-container w3-border-top w3-padding-16 w3-light-grey w3-round">
					...
					<a href="minadminprofil.php" type="button" class="w3-button w3-red w3-round">Cancel</a>
				</div>
	...
	<div id="delete-modal" class="w3-modal">
		<div class="w3-modal-content w3-card-4 w3-animate-zoom" style="max-width:600px">
			<div class="w3-center"><br>
				<span onclick="document.getElementById('delete-modal').style.display='none'" class="w3-button w3-xlarge w3-transparent w3-display-topright" title="Close Modal">×</span> <img src="Assets/logo-black.png" alt="Logo" style="width:30%" class="w3-margin-top"> </div>
...
<?php 
	$uid = $_SESSION['uid'];
	$sql = "SELECT * FROM offentligcomments WHERE orderid=$orderid;";
	$result = mysqli_query($conn, $sql);
	...
	if(isset($row['uid'])){
			$userid=$row['uid'];
			$usersql = "SELECT * FROM users WHERE uid=$userid;";
		...
		echo '
			  ...
			  <div class="panel w3-pale-blue">'.$row['comment'].'
			  </div>
			  </div>';
				}
			...
			$adminrow=mysqli_fetch_assoc($adminresult);
			  ...
			  <div class="panel w3-pale-green">'.$row['comment'].'
			  </div>
			  </div>';
			}
			}
...
<div class="w3-padding-16"></div>
	 <form action="includes/createcomment.inc.php" class="container w3-bottom w3-margin-left" style="position: fixed" method="post">
		 <div class="w3-section">
