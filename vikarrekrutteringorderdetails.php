...
<body class="w3-animate-opacity">
	...
	<div id="id03" class="w3-half">
			...
			<div class="w3-section">
				<form class="w3-container" action="includes/updatevikarrekrutteringorder.inc.php" method="post">
						
					<?php 
						$sql ="SELECT * FROM vikarrekruttering WHERE orderid=$orderid";
					...
					<input disabled class="w3-input w3-border w3-hover-border-black w3-section" type="text" name="virksomhedsnavn" value="<?php echo $row['virksomhedsnavn']; ?>" style="border: 0px">
					<label>Kontaktperson</label>
					<input class="w3-input w3-border w3-hover-border-black w3-section" type="text" name="kontaktperson" value="<?php echo $row['kontaktperson']; ?>" style="border: 0px">
					...
					<input type="text" class="w3-input w3-border w3-hover-border-black w3-section" name="varighed" value="<?php echo $row['varighed']; ?>" style="border: 0px">
					<label>antal vikarer</label>
					<input class="w3-input w3-border w3-hover-border-black w3-section" type="text" name="antal" value="<?php echo $row['antal']; ?>">
					<label>(valgfri) Giv en kort beskrivelse af arbjedsopgaven</label>
					<textarea value="<?php echo $row['beskrivelse']; ?>" rows="4" cols="50" class="w3-input w3-border w3-hover-border-black w3-section" name="beskrivelse"><?php echo $row['beskrivelse']; ?></textarea>
					<?php echo' <input type="hidden" name="orderid" value='.$orderid.'> ';?>
					<label>Type af opgave</label>
					<input disabled class="w3-input w3-border w3-hover-border-black w3-section" type="text" name="typeafopgave" value="<?php echo $row['typeafopgave']; ?>">
					<label>Mødetid</label>
					<input class="w3-input w3-border w3-hover-border-black w3-section" type="time" name="moedetid" value="<?php echo $row['moedetid']; ?>">
					<label>Opgave adresse</label>
					<input disabled class="w3-input w3-border w3-hover-border-black w3-section" type="text" name="opgaveadresse" value="<?php echo $row['opgaveadresse']; ?>">
					<label>Firma adresse</label>
					<input disabled class="w3-input w3-border w3-hover-border-black w3-section" type="text" name="firmaadresse" value="<?php echo $row['firmaadresse']; ?>">
					<label>Faktura email</label>
					<input disabled class="w3-input w3-border w3-hover-border-black w3-section" type="email" name="fakturaemail" value="<?php echo $row['fakturaemail']; ?>">
					<label>EAN Nr.(Valgfrit)</label>
					<input class="w3-input w3-border w3-hover-border-black w3-section" type="text" name="ean" value="<?php echo $row['ean']; ?>">
					<label>Hjemmeside</label>
					<input class="w3-input w3-border w3-hover-border-black w3-section" type="text" name="website" value="<?php echo $row['website']; ?>">
					...
					<select class="w3-section w3-input w3-border w3-hover-border-blue" name="adminuid">
						...
						while($row2 =  mysqli_fetch_assoc($result2)){
							$adminuid=$row2['adminuid'];
							$adminname=$row2['adminname'];
							$adminemail = $_SESSION['email'];
							if(strpos($adminemail, "ma@danpanel.dk")!==false){
							echo '<option class="w3-bar-item w3-button" name="adminuid" value='.$adminuid.'>'.$adminname.'</option>';
						}
						}
				...
				<form action="includes/uploadvikarrekruttering.inc.php" method="post" enctype="multipart/form-data">
					<?php echo' <input type="hidden" name="orderid" value='.$orderid.'> ';?>
				...
				<th>Funktion</th>
				<?php 
				$sql = "SELECT * FROM vikarrekrutteringuserfiles WHERE orderid=$orderid;";
				$result = mysqli_query($conn, $sql);
	...
	<div id="delete-modal" class="w3-modal">
		<div class="w3-modal-content w3-card-4 w3-animate-zoom w3-round" style="max-width:600px">
			<div class="w3-center"><br>
				...
				<form action="includes/deletevikarekrutteringorder.inc.php" method="post">
				<?php echo' <input type="hidden" name="orderid" value='.$orderid.'> ';?>	
			...
			<div class="w3-container w3-border-top w3-padding-16 w3-light-grey w3-round">
				...
				<div class="w3-container w3-border-top w3-padding-16 w3-light-grey">
					<a href="minprofil.php" type="button" class="w3-button w3-red w3-round">Cancel</a>
				</div>
			</div>
...
<?php 
	$uid = $_SESSION['uid'];
	$sql = "SELECT * FROM vikarrekrutteringcomments WHERE orderid=$orderid;";
	$result = mysqli_query($conn, $sql);
	$resultCheck = mysqli_num_rows($result);
	if ($resultCheck > 0){
...
<div class="w3-padding-16"></div>
	 <form action="includes/createvikarrekrutteringcomments.inc.php" class="container w3-bottom w3-margin-left" style="position: fixed" method="post">
		 <div class="w3-section">
