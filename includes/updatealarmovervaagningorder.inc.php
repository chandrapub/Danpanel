...
if(isset($_POST['updateorder-submit'])){
  // We grab all the data which we passed from the signup form so we can use it later.
	...
	$sagsnr = $_POST['sagsnr'];
        ...
        $sql = "UPDATE alarmovervaagning SET kontaktperson=?, ean=?, website=?, adminuid=?, progressionoforder=? WHERE orderid=$orderid;";
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          // If there is an error we send the user back to the signup page.
				 ...
				 header("Location: ../alarmovervaagningorderdetails.php?updateorder=error&orderid=".$orderid);
          exit();
		...
		$sqlbefore = "SELECT * FROM alarmovervaagning WHERE orderid=$orderid";
		...
		if($website!==$rowbefore['website']){
			...
			$websitechange = "hjemmesiden er ikke ændret";
				...
				if(!mysqli_stmt_bind_param($stmt, "sssii", $kontaktperson, $ean, $website, $adminuid, $progressionoforder)){
					header("Location: ../alarmovervaagningorderdetails.php?updateorder=error1&orderid=".$orderid);
          			exit();
				}
				if(!mysqli_stmt_execute($stmt)){
					header("Location: ../alarmovervaagningorderdetails.php?updateorder=error2&orderid=".$orderid);
          			exit();
				}
				...
				$txt = "sagsnummeret: ".$rowbefore['sagsnr']." er blevet opdateret af ".$nameto. ".\n\nÆndringerne er følgende: ".$kontaktpersonchange." || ". $eanchange." || ".$websitechange;
				
		if(strpos($_SESSION['email'],'@danpanel.dk')!==false){
				if($emailto==$_SESSION['email']){
					mail("ma@danpanel.dk", $subject, $txt);
					header("Location: ../alarmovervaagningorderdetails.php?updateorder=success&orderid=".$orderid."&noemailsentto=".$emailto);
          			exit();
					}
					mail("ma@danpanel.dk", $subject, $txt);
					mail($emailto, $subject, $txt);
					header("Location: ../alarmovervaagningorderdetails.php?updateorder=success&orderid=".$orderid."&emailsentto=".$emailto);
					exit();
					}
				else{
				mail("ma@danpanel.dk", $subject, $txt);
