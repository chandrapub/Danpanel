...
if (isset($_POST['edit-submit'])) {
  ...
  if (empty($name) || empty($email) || empty($phonenr)) {
    header("Location: ../min".$_SESSION['typeofuser']."profil.php?error=emptyfields&uid=".$name."&email=".$email);
    exit();
  }
  // We check for an invalid username AND invalid e-mail.
    ...
    header("Location: ../min".$_SESSION['typeofuser']."profil.php?error=invalidname&email");
    exit();
  }
  // We check for an invalid username. In this case ONLY letters and numbers.
  else if (!preg_match("/^[a-zA-Z0-9' ']*$/", $name)) {
    header("Location: ../min".$_SESSION['typeofuser']."profil.php?error=invalidname&email=".$email."&phonenr=".$phonenr);
    exit();
  }
  // We check for an invalid e-mail.
  else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    header("Location: ../min".$_SESSION['typeofuser']."profil.php?error=invalidemail&name=".$name."&phonenr=".$phonenr);
    exit();
  }
	else if (!preg_match('/^[0-9]{8}+$/', $phonenr)) {
    header("Location: ../min".$_SESSION['typeofuser']."profil.php?error=invalidphonenr&name=".$name."&email=".$email);
    exit();
	}
  // We check if the repeated password is NOT the same.
    ...
    // We also need to include another error handler here that checks whether or the username is already taken. We HAVE to do this using prepared statements because it is safer!
        ...
        $sql = "UPDATE users SET name=?, email=?, phonenr=? WHERE uid=$uid;";
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          ...
          mysqli_stmt_bind_param($stmt, "ssi",$name, $email,$phonenr);
          // Then we execute the prepared statement and send it to the database!
          // This means the user is now registered! :)
          mysqli_stmt_execute($stmt);
          // Lastly we send the user back to the my profile page with a success message!
	...
	$_SESSION['phonenr'] = $row['phonenr'];
			if	(strpos($email, "@danpanel.dk")){
				 header("Location: ../minadminprofil.php?edit=success");
		  
          exit();
			}
			else
          header("Location: ../min".$_SESSION['typeofuser']."profil.php?edit=success");
