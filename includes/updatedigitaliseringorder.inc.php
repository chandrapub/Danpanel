...
if(isset($_POST['updateorder-submit'])){
  // We grab all the data which we passed from the signup form so we can use it later.
	...
	$sagsnr = $_POST['sagsnr'];
        ...
        $sql = "UPDATE digitalisering SET kontaktperson=?, beskrivelse=?, ean=?, website=?, adminuid=?, progressionoforder=? WHERE orderid=$orderid;";
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          // If there is an error we send the user back to the signup page.
				 ...
				 header("Location: ../digitaliseringorderdetails.php?updateorder=error&orderid=".$orderid);
          exit();
          ...
          // The hashing method I am going to show here, is the LATEST version and will always will be since it updates automatically. DON'T use md5 or sha256 to hash, these are old and outdated!
			...
			$sqlbefore = "SELECT * FROM digitalisering WHERE orderid=$orderid";
		...
		if($website!==$rowbefore['website']){
			...
			$websitechange = "hjemmesiden er ikke ændret";
				...
				if(!mysqli_stmt_bind_param($stmt, "ssssii", $kontaktperson, $beskrivelse, $ean, $website, $adminuid, $progressionoforder)){
					header("Location: ../digitaliseringorderdetails.php?updateorder=error1&orderid=".$orderid);
          			exit();
				}
				if(!mysqli_stmt_execute($stmt)){
					header("Location: ../digitaliseringorderdetails.php?updateorder=error2&orderid=".$orderid);
          			exit();
				}
