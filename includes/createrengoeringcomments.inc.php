...
if (isset($_POST['createcomment-submit'])) {
  ...
  // We grab all the data which we passed from the signup form so we can use it later.
	...
	$orderid = $_POST['orderid'];
        ...
        $sql = "INSERT INTO rengoeringcomments (commenttext, adminuid, orderid) VALUES (?, ?, ?);";
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          ...
          header("Location: ../rengoeringorderdetailsrengoeringorderdetails.php?error=sqlerror");
          exit();
        }
          ...
          header("Location: ../adminrengoeringorderdetails.php?createcomment=success&orderid=".$orderid);
          exit();
		}
        }
		$sql = "INSERT INTO rengoeringcomments (commenttext, uid, orderid) VALUES (?, ?, ?);";
		$stmt = mysqli_stmt_init($conn);
        // Then we prepare our SQL statement AND check if there are any errors with it.
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          // If there is an error we send the user back to the signup page.
          ...
          // Lastly we send the user back to the signup page with a success message!
				 ...
				 header("Location: ../rengoeringorderdetails.php?updateorder=success&orderid=".$orderid);
          exit();
