...
if ( empty( empty( $kontaktperson ) || $typeafopgave ) || empty( $sagsnr ) || empty( $opgaveadresse ) || empty( $kvadratmeter ) || empty( $beskrivelse) ) {
	...
	// We check for an invalid username.
        ...
        $sql = "INSERT INTO rengoering (kontaktperson, sagsnr, opgavetype, opgaveadresse, kvadratmeter, beskrivelse, website, uid) VALUES (?, ?, ?, ?, ?, ?, ?, ?);";
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          ...
          mysqli_stmt_bind_param($stmt, "ssssissi", $kontaktperson, $sagsnr, $typeafopgave, $opgaveadresse, $kvadratmeter, $beskrivelse, $website, $uid);
          ...
          header("Location: ../minadminprofil.php?creatorder=success");
          exit();
			}
			else{
			mail("ma@danpanel.dk", $subject, $txt);
