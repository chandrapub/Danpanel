...
if(isset($_POST['updateorder-submit'])){
  // We grab all the data which we passed from the signup form so we can use it later.
	...
	$orderid = $_POST['orderid'];
        ...
        $sql = "UPDATE vikarrekruttering SET kontaktperson=?, beskrivelse=?, antal=?, varighed=?, moedetid=?, ean=?, website=?, adminuid=?, progressionoforder=? WHERE orderid=$orderid;";
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          // If there is an error we send the user back to the signup page.
				 ...
				 header("Location: ../vikarrekrutteringorderdetails.php?updateorder=error&orderid=".$orderid);
          exit();
          ...
          // The hashing method I am going to show here, is the LATEST version and will always will be since it updates automatically. DON'T use md5 or sha256 to hash, these are old and outdated!
			...
			$sqlbefore = "SELECT * FROM vikarrekruttering WHERE orderid=$orderid";
		...
		if($kontaktperson!==$rowbefore['kontaktperson']){
			...
			if($varighed!==$rowbefore['varighed']){
			$varighedchange = "varighed er blevet ændret fra ".$rowbefore['varighed']. " til ".$varighed;
		}else{
			$varighedchange = "varighed er ikke ændret";
		}
				if($antal!==$rowbefore['antal']){
			$antalchange = "antal er blevet ændret fra ".$rowbefore['antal']. " til ".$antal;
		}else{
			$antalchange = "antal er ikke ændret";
		}
					if($moede!==$rowbefore['moede']){
			$moedechange = "moede er blevet ændret fra ".$rowbefore['moede']. " til ".$moede;
		}else{
			$moedechange = "moede er ikke ændret";
		}
		...
		if($beskrivelse!==$rowbefore['beskrivelse']){
			...
			if($ean!==$rowbefore['ean']){
			$eanchange = "ean nummeret er blevet ændret fra ".$rowbefore['ean']. " til ".$ean;
		}else{
			$eanchange = "ean nummeret er ikke ændret";
		}
		...
		if($website!==$rowbefore['website']){
			...
			$websitechange = "hjemmesiden er ikke ændret";
				...
				if(!mysqli_stmt_bind_param($stmt, "ssiisssii", $kontaktperson, $beskrivelse, $antal, $varighed, $moedetid, $ean, $website, $adminuid, $progressionoforder)){
					header("Location: ../vikarrekrutteringorderdetails.php?updateorder=error1&orderid=".$orderid);
          			exit();
				}
				if(!mysqli_stmt_execute($stmt)){
					header("Location: ../vikarrekrutteringorderdetails.php?updateorder=error2&orderid=".$orderid);
          			exit();
				}
				...
				$txt = "sagsnummeret: ".$rowbefore['sagsnr']." er blevet opdateret af ".$nameto. ".\n\nÆndringerne er følgende: ".$kontaktpersonchange." || ". $eanchange." || ".$varighedchange. " || " . $antalchange ." || ". $beskrivelsechange. " || ". $moedechange. " || ".$eanchange. " || ". $websitechange;
				
		if(strpos($_SESSION['email'],'@danpanel.dk')!==false){
				if($emailto==$_SESSION['email']){
					mail("ma@danpanel.dk", $subject, $txt);
					header("Location: ../vikarrekrutteringorderdetails.php?updateorder=success&orderid=".$orderid."&noemailsentto=".$emailto);
          			exit();
					}
					mail("ma@danpanel.dk", $subject, $txt);
					mail($emailto, $subject, $txt);
					header("Location: ../vikarrekrutteringorderdetails.php?updateorder=success&orderid=".$orderid."&emailsentto=".$emailto);
					exit();
					}
				else{
				mail("ma@danpanel.dk", $subject, $txt);
