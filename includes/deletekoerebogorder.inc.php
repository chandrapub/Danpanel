...
if(isset($_POST['deleteorder-submit'])){
		...
		$sql = "DELETE FROM koerebog WHERE orderid=?;";
		
		$sqlGetAllFiles ="SELECT * FROM koereboguserfiles WHERE orderid=$orderid;";
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
			if(strpos($_SESSION['email'],'@danpanel.dk')){
				header("Location: ../koerebogorderdetails.php");
					exit();
			}
			else{
...
if(strpos($_SESSION['email'],'@danpanel.dk')){
	...
	header("Loacation: ../koerebogorderdetails.php");
	exit();
}
else{
	header("Loacation: ../koerebogorderdetails.php");
