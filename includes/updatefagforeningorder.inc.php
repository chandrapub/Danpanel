...
if(isset($_POST['updateorder-submit'])){
  // We grab all the data which we passed from the signup form so we can use it later.
	...
	$orderid = $_POST['orderid'];
        ...
        $sql = "UPDATE fagforening SET adminuid=?, progressionoforder=? WHERE orderid=$orderid;";
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          // If there is an error we send the user back to the signup page.
				 ...
				 header("Location: ../fagforeningorderdetails.php?updateorder=error&orderid=".$orderid);
          exit();
		...
		$sqlbefore = "SELECT * FROM fagforening WHERE orderid=$orderid";
		...
		$rowbefore = mysqli_fetch_assoc($resultbefore);
				...
				if(!mysqli_stmt_bind_param($stmt, "ii", $adminuid, $progressionoforder)){
					header("Location: ../fagforeningorderdetails.php?updateorder=error1&orderid=".$orderid);
          			exit();
				}
				if(!mysqli_stmt_execute($stmt)){
					header("Location: ../fagforeningorderdetails.php?updateorder=error2&orderid=".$orderid);
          			exit();
				}
		...
		if(strpos($_SESSION['email'],'@danpanel.dk')!==false){
				if($emailto==$_SESSION['email']){
					...
					header("Location: ../fagforeningorderdetails.php?updateorder=success&orderid=".$orderid."&noemailsentto=".$emailto);
          			exit();
					}
					mail("ma@danpanel.dk", $subject, $txt);
					mail($emailto, $subject, $txt);
					header("Location: ../fagforeningorderdetails.php?updateorder=success&orderid=".$orderid."&emailsentto=".$emailto);
					exit();
					}
				else{
				mail("ma@danpanel.dk", $subject, $txt);
