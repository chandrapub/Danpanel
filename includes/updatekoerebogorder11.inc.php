...
if(isset($_POST['updateorder-submit'])){
  // We grab all the data which we passed from the signup form so we can use it later.
	...
	// $antal = $_POST['antal'];
		...
		// $sql = "UPDATE koerebog SET kontaktperson=?, beskrivelse=?, website=?, adminuid=?, progressionoforder=? WHERE orderid=$orderid;";
		$sql = "UPDATE koerebog SET kontaktperson='$kontaktperson', beskrivelse='$beskrivelse', website='$website', adminuid=$adminuid, progressionoforder=$progressionoforder WHERE orderid=$orderid;";
		...
		if($result){
			...
			$sqlbefore = "SELECT * FROM koerebog WHERE orderid=$orderid";
		...
		if($website!==$rowbefore['website']){
			...
			header("Location: ../koerebogorderdetails.php?updateorder=error&orderid=".$orderid);
			//   exit();
		...
		// }
				...
				if(!mysqli_stmt_bind_param($stmt, "sssii", $kontaktperson, $beskrivelse, $website, $adminuid, $progressionoforder)){
					header("Location: ../koerebogorderdetails.php?updateorder=error1&orderid=".$orderid);
          			exit();
				}
				if(!mysqli_stmt_execute($stmt)){
					header("Location: ../koerebogorderdetails.php?updateorder=error2&orderid=".$orderid);
          			exit();
				}
				...
				$subject = "Opdatering til sag ".$rowbefore['sagsnr']. "af typen privat/erhverv";
				$txt = "sagsnummeret: ".$rowbefore['sagsnr']." er blevet opdateret af ".$nameto. " Ordre opdateret: Medarbejder sat på sagen:". $row['adminname']. "|| Ordre status ændret fra" .$rowbefore['progressionoforder']." til $progressionoforder";
				
		if(strpos($_SESSION['email'],'@danpanel.dk')!==false){
				if($emailto==$_SESSION['email']){
					mail("ma@danpanel.dk", $subject, $txt);
					header("Location: ../koerebogorderdetails.php?updateorder=success&orderid=".$orderid."&noemailsentto=".$emailto);
          			exit();
					}
					mail("ma@danpanel.dk", $subject, $txt);
					mail($emailto, $subject, $txt);
					header("Location: ../koerebogorderdetails.php?updateorder=success&orderid=".$orderid."&emailsentto=".$emailto);
					exit();
					}
				else{
				mail("ma@danpanel.dk", $subject, $txt);
