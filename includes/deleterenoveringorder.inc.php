...
if(isset($_POST['deleteorder-submit'])){
		...
		$sql = "DELETE FROM renovering WHERE orderid=?;";
		
		$sqlGetAllFiles ="SELECT * FROM renoveringuserfiles WHERE orderid=$orderid;";
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
			if(strpos($_SESSION['email'],'@danpanel.dk')){
				header("Location: ../renoveringorderdetails.php");
					exit();
			}
			else{
...
if(strpos($_SESSION['email'],'@danpanel.dk')){
	...
	header("Loacation: ../renoveringorderdetails.php");
	exit();
}
else{
	header("Loacation: ../renoveringorderdetails.php");
