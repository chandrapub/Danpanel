...
if(isset($_POST['updateorder-submit'])){
  // We grab all the data which we passed from the signup form so we can use it later.
	...
	$orderid = $_POST['orderid'];
        ...
        $sql = "UPDATE renovering SET beskrivelse=?, adminuid=?, progressionoforder=? WHERE orderid=$orderid;";
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          // If there is an error we send the user back to the signup page.
				 ...
				 header("Location: ../renoveringorderdetails.php?updateorder=error&orderid=".$orderid);
          exit();
          ...
          // The hashing method I am going to show here, is the LATEST version and will always will be since it updates automatically. DON'T use md5 or sha256 to hash, these are old and outdated!
			...
			$sqlbefore = "SELECT * FROM renovering WHERE orderid=$orderid";
		...
		if($beskrivelse!==$rowbefore['beskrivelse']){
			...
			$beskrivelsechange = "beskrivelse nummeret er ikke ændret";
				...
				if(!mysqli_stmt_bind_param($stmt, "sii", $beskrivelse, $adminuid, $progressionoforder)){
					header("Location: ../renoveringorderdetails.php?updateorder=error1&orderid=".$orderid);
          			exit();
				}
				if(!mysqli_stmt_execute($stmt)){
					header("Location: ../renoveringorderdetails.php?updateorder=error2&orderid=".$orderid);
          			exit();
				}
		...
		if(strpos($_SESSION['email'],'@danpanel.dk')!==false){
				if($emailto==$_SESSION['email']){
					...
					header("Location: ../renoveringorderdetails.php?updateorder=success&orderid=".$orderid."&noemailsentto=".$emailto);
          			exit();
					}
					mail("ma@danpanel.dk", $subject, $txt);
					mail($emailto, $subject, $txt);
					header("Location: ../renoveringorderdetails.php?updateorder=success&orderid=".$orderid."&emailsentto=".$emailto);
					exit();
					}
				else{
				mail("ma@danpanel.dk", $subject, $txt);
