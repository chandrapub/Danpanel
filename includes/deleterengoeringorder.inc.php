...
if(isset($_POST['deleteorder-submit'])){
		...
		$sql = "DELETE FROM rengoering WHERE orderid=?;";
		
		$sqlGetAllFiles ="SELECT * FROM rengoeringuserfiles WHERE orderid=$orderid;";
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
			if(strpos($_SESSION['email'],'@danpanel.dk')){
				header("Location: ../rengoeringorderdetails.php");
					exit();
			}
			else{
...
if(strpos($_SESSION['email'],'@danpanel.dk')){
	...
	header("Loacation: ../rengoeringorderdetails.php");
	exit();
}
else{
	header("Loacation: ../rengoeringorderdetails.php");
