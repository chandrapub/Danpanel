...
$amountprmonth = $_POST['amountprmonth'];
$monthleftoncurrentdeal = $_POST['monthleftoncurrentdeal'];
$amountofcoffeprmonth = $_POST['amountofcoffeprmonth'];
$priceprkgcoffe = $_POST['priceprkgcoffe'];
$typeoffutureproducts = $_POST['typeoffutureproducts'];
...
if ( empty( empty( $currentsolution ) || $amountprmonth ) || empty( $sagsnr ) || empty( $monthleftoncurrentdeal ) || empty( $amountofcoffeprmonth ) || empty( $priceprkgcoffe ) || empty( $typeoffutureproducts ) ) {
	...
	// We check for an invalid username AND invalid e-mail.
        ...
        $sql = "INSERT INTO kaffeloesning (sagsnr, currentsolution, amountprmonth, monthleftoncurrentdeal, amountofcoffeprmonth, priceprkgcoffe, typeoffutureproducts, uid) VALUES (?, ?, ?, ?, ?, ?, ?, ?);";
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          ...
          mysqli_stmt_bind_param($stmt, "sssssssi",$sagsnr, $currentsolution, $amountprmonth, $monthleftoncurrentdeal, $amountofcoffeprmonth, $priceprkgcoffe, $typeoffutureproducts, $uid);
          // Then we execute the prepared statement and send it to the database!
          // This means the user is now registered! :)
          mysqli_stmt_execute($stmt);
			
			$subject = "Sag oprettet ".$sagsnr. "af typen erhverv";
				$txt = "sagsnummeret: ".$sagsnr;
          ...
          exit();} else{
				mail("ma@danpanel.dk", $subject, $txt);
				header("Location: ../min".$_SESSION['typeofuser']."profil.php?creatorder=success");
				exit();
			}
