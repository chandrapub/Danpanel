...
if(isset($_POST['deleteorder-submit'])){
		...
		$sql = "DELETE FROM vikarrekruttering WHERE orderid=$orderid;";
		
		$sqlGetAllFiles ="SELECT * FROM vikarrekrutteringuserfiles WHERE orderid=$orderid;";
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
			if(strpos($_SESSION['email'],'@danpanel.dk')){
				header("Location: ../vikarrekrutteringorderdetails.php");
					exit();
			}
			else{
...
if(strpos($_SESSION['email'],'@danpanel.dk')){
	...
	header("Loacation: ../vikarrekrutteringorderdetails.php");
	exit();
}
else{
	header("Loacation: ../vikarrekrutteringorderdetails.php");
