...
if(isset($_POST['deleteorder-submit'])){
		...
		$sql = "DELETE FROM fagforening WHERE orderid=?;";
		
		$sqlGetAllFiles ="SELECT * FROM fagforeninguserfiles WHERE orderid=$orderid;";
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
			if(strpos($_SESSION['email'],'@danpanel.dk')){
				header("Location: ../fagforeningorderdetails.php");
					exit();
			}
			else{
...
if(strpos($_SESSION['email'],'@danpanel.dk')){
	...
	header("Loacation: ../fagforeningorderdetails.php");
	exit();
}
else{
	header("Loacation: ../fagforeningorderdetails.php");
