...
if (isset($_POST['createcomment-submit'])) {
  ...
  // We grab all the data which we passed from the signup form so we can use it later.
	...
	$orderid = $_POST['orderid'];
        ...
        $sql = "INSERT INTO digitaliseringcomments (commenttext, adminuid, orderid) VALUES (?, ?, ?);";
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          ...
          header("Location: ../digitaliseringdigitaliseringorderdetails.php?error=sqlerror");
          exit();
        }
          ...
          header("Location: ../admindigitaliseringorderdetails.php?createcomment=success&orderid=".$orderid);
          exit();
		}
        }
		$sql = "INSERT INTO digitaliseringcomments (commenttext, uid, orderid) VALUES (?, ?, ?);";
		$stmt = mysqli_stmt_init($conn);
        // Then we prepare our SQL statement AND check if there are any errors with it.
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          // If there is an error we send the user back to the signup page.
          ...
          mysqli_stmt_execute($stmt);
          // Last
				 header("Location: ../digitaliseringorderdetails.php?updateorder=success&orderid=".$orderid);
          exit();
