...
if(isset($_POST['deleteorder-submit'])){
		...
		$sql = "DELETE FROM erhvervlejemaal WHERE orderid=?;";
		
		$sqlGetAllFiles ="SELECT * FROM erhvervlejemaaluserfiles WHERE orderid=$orderid;";
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
			if(strpos($_SESSION['email'],'@danpanel.dk')){
				header("Location: ../erhvervlejemaalorderdetails.php");
					exit();
			}
			else{
...
if(strpos($_SESSION['email'],'@danpanel.dk')){
	...
	header("Loacation: ../erhvervlejemaalorderdetails.php");
	exit();
}
else{
	header("Loacation: ../erhvervlejemaalorderdetails.php");
