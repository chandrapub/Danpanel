...
<body class="w3-animate-opacity">
	...
	<div id="id03" class="w3-half">
			...
			<div class="w3-section">
				<form class="w3-container" action="includes/updatekoerebogorder.inc.php" method="post">
						
					<?php 
						$sql ="SELECT * FROM koerebog WHERE orderid=$orderid";
					...
					<label>Antal</label>
					<!-- <input disabled class="w3-input w3-border w3-hover-border-black w3-section" type="text" name="typeafopgave" value="<?php echo $row['antal']; ?>"> -->
					<input disabled class="w3-input w3-border w3-hover-border-black w3-section" type="text" name="antal" value="<?php echo $row['antal']; ?>">
					<label>Møde</label>
					<input disabled class="w3-input w3-border w3-hover-border-black w3-section" type="datetime" name="kvadratmeter" value="<?php echo $row['moede']; ?>">
				...
				<form action="includes/uploadkorebog.inc.php" method="post" enctype="multipart/form-data">
					<?php echo' <input type="hidden" name="orderid" value='.$orderid.'> ';?>
				...
				<th>Funktion</th>
				<?php 
				$sql = "SELECT * FROM koereboguserfiles WHERE orderid=$orderid;";
				$result = mysqli_query($conn, $sql);
	...
	<div id="delete-modal" class="w3-modal">
		<div class="w3-modal-content w3-card-4 w3-animate-zoom w3-round" style="max-width:600px">
			<div class="w3-center"><br>
				...
				<form action="includes/deletekoerebogorder.inc.php" method="post">
				<?php echo' <input type="hidden" name="orderid" value='.$orderid.'> ';?>	
...
<?php 
	$uid = $_SESSION['uid'];
	$sql = "SELECT * FROM koerebogcomments WHERE orderid=$orderid;";
	$result = mysqli_query($conn, $sql);
	$resultCheck = mysqli_num_rows($result);
	if ($resultCheck > 0){
...
<div class="w3-padding-16"></div>
	 <form action="includes/createkoerebogcomments.inc.php" class="container w3-bottom w3-margin-left" style="position: fixed" method="post">
		 <div class="w3-section">
