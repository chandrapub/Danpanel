...
<body class="w3-animate-opacity">
	...
	<div id="id03" class="w3-half">
			...
			<div class="w3-section">
				<form class="w3-container" action="includes/updateerhvervlejemaalorder.inc.php" method="post">
						
					<?php 
						$sql ="SELECT * FROM erhvervlejemaal WHERE orderid=$orderid";
					...
					<label>Giv en kort beskrivelse</label>
					<textarea rows="4" cols="50" class="w3-input w3-border w3-hover-border-black w3-section" name="beskrivelse"><?php echo $row['beskrivelse']; ?></textarea>
					<?php echo' <input type="hidden" name="orderid" value='.$orderid.'> ';?>
					<label>Type af lokale</label>
					<input disabled class="w3-input w3-border w3-hover-border-black w3-section" type="text" name="typeafopgave" value="<?php echo $row['typeaflokale']; ?>">
					<label>Budget</label>
					<textarea disabled class="w3-input w3-border w3-hover-border-black w3-section" name="budget"><?php echo $row['budget']; ?></textarea>
					<label>Firma adresse</label>
					<input disabled class="w3-input w3-border w3-hover-border-black w3-section" type="text" name="opgaveadresse" value="<?php echo $row['firmaadresse']; ?>">
					<label>Kvadratmeter</label>
					<input disabled class="w3-input w3-border w3-hover-border-black w3-section" type="text" name="kvadratmeter" value="<?php echo $row['kvadratmeter']; ?>">
				...
				<form action="includes/uploaderhvervslejemaal.inc.php" method="post" enctype="multipart/form-data">
					<?php echo' <input type="hidden" name="orderid" value='.$orderid.'> ';?>
				...
				<th>Funktion</th>
				<?php 
				$sql = "SELECT * FROM erhvervlejemaaluserfiles WHERE orderid=$orderid;";
				$result = mysqli_query($conn, $sql);
	...
	<div id="delete-modal" class="w3-modal">
		<div class="w3-modal-content w3-card-4 w3-animate-zoom w3-round" style="max-width:600px">
			<div class="w3-center"><br>
				...
				<form action="includes/deleteerhvervlejemaalorder.inc.php" method="post">
				<?php echo' <input type="hidden" name="orderid" value='.$orderid.'> ';?>	
...
<?php 
	$uid = $_SESSION['uid'];
	$sql = "SELECT * FROM erhvervlejemaalcomments WHERE orderid=$orderid;";
	$result = mysqli_query($conn, $sql);
	$resultCheck = mysqli_num_rows($result);
	if ($resultCheck > 0){
...
<div class="w3-padding-16"></div>
	 <form action="includes/createerhvervlejemaalcomments.inc.php" class="container w3-bottom w3-margin-left" style="position: fixed" method="post">
		 <div class="w3-section">
